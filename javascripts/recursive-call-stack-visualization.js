(() => {
  class RecursiveCallStackVisualizer {
    ...
    prepare() {
      ...
      for (let value = n; value >= 0; value -= 1) {
        ...
        this.steps.push(this.snapshot(frames, 'expansion', value === 0 ? 2 : 4,
          value === 0
            ? 'The base case has been reached. No smaller call is needed.'
            : `Push a frame for n = ${value}. Multiplication by ${value} is suspended until the recursive call returns.`,
          null, 0));
      ...
      for (let value = 1; value <= n; value += 1) {
        ...
        result *= value;
        operations += 1;
        this.steps.push(this.snapshot(frames, 'unwinding', 4,
          `Resume the frame for n = ${value}: ${value} × ${previous} = ${result}. Then pop that frame.`,
          result, operations));
        frames.pop();
      }
    ...
    render(isError = false) {
      ...
      stack.innerHTML = state.frames.length
        ? [...state.frames].reverse().map((frame, reverseIndex) => {
            ...
            return `<article class="${classes.join(' ')}">
              <div><strong>factorial(${frame.n})</strong><small>${actualIndex === state.frames.length - 1 ? 'top frame' : 'suspended frame'}</small></div>
              <code>n = ${frame.n}</code>
      ...
      this.root.querySelectorAll('[data-line]').forEach((line) => {
        line.classList.toggle('is-current', Number(line.dataset.line) === state.line);
      });
    ...
    labelForPhase(phase) {
      return ({
        start: 'Initial call',
        expansion: 'Expansion: push a frame',
        'base case': 'Base case',
        unwinding: 'Unwinding: resume and pop',
        complete: 'Completed',
        error: 'Invalid input'
      })[phase] || phase;
    }
  ...
  function mountDemo() {
    ...
    const heading = [...document.querySelectorAll('.md-content h2')]
      .find((node) => node.textContent.includes('Expansion and unwinding'));
    if (!heading) return;
