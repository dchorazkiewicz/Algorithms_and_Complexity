(() => {
  ...
  class ArrayShiftVisualizer {
    ...
    prepare() {
      ...
      if (this.operation.value === 'insert') this.prepareInsertion(index, Number(this.valueInput.value));
    ...
    togglePlay() { if (!this.steps.length) this.prepare(); if (this.timer) return this.stop(); this.root.querySelector('[data-action="play"]').textContent = 'Pause'; this.timer = window.setInterval(() => { if (this.stepIndex >= this.steps.length - 1) return this.stop(); this.stepIndex += 1; this.render(); }, 900); }
    stop() { if (this.timer) window.clearInterval(this.timer); this.timer = null; const button = this.root.querySelector('[data-action="play"]'); if (button) button.textContent = 'Play'; }
  ...
  function loadAsset(kind, path, marker) {
    ...
    if (kind === 'css') { element.rel = 'stylesheet'; element.href = `${siteRoot()}/${path}`; }
    else element.src = `${siteRoot()}/${path}`;
  ...
  function mountArrayShift() {
    ...
    root.dataset.arrayShiftViz = '';
    root.dataset.values = '10,20,30,40,_';
    heading.parentNode.insertBefore(root, heading);
  }

  function mountExternal(pathSuffix, headingText, dataAttribute, cssFile, jsFile, marker) {
    const path = window.location.pathname.replace(/\/$/, '');
    if (!path.endsWith(pathSuffix)) return;
    loadAsset('css', `stylesheets/${cssFile}`, `${marker}-css`);
    if (!document.querySelector(`[${dataAttribute}]`)) {
      const heading = [...document.querySelectorAll('.md-content h2')].find(node => node.textContent.includes(headingText));
      if (heading) {
        const root = document.createElement('div');
    ...
    loadAsset('js', `javascripts/${jsFile}`, `${marker}-js`);
  }

  function initialise() {
    mountArrayShift();
    mountExternal('/01-arrays-as-indexed-structures', 'Direct access', 'data-array-access-viz', 'array-access-traversal-visualization.css', 'array-access-traversal-visualization.js', 'array-access');
    mountExternal('/04-anatomy-of-a-recursive-call', 'Expansion and unwinding', 'data-recursive-stack-viz', 'recursive-call-stack-visualization.css', 'recursive-call-stack-visualization.js', 'recursive-stack');
