(() => {
  ...
  class ArrayShiftVisualizer {
    ...
    renderShell() {
      ...
      this.root.innerHTML = `
        <div class="ds-viz__header">
          <div><p class="ds-viz__eyebrow">Interactive trace</p><h3>Array insertion and deletion</h3></div>
          <span class="ds-viz__complexity" aria-live="polite">Cost: —</span>
        </div>
        <div class="ds-viz__controls">
          <label>Operation<select data-role="operation"><option value="insert">Insert</option><option value="delete">Delete</option></select></label>
          <label>Index<input data-role="index" type="number" min="0" value="2"></label>
          <label data-role="value-label">Value<input data-role="value" type="number" value="25"></label>
        ...
        <div class="ds-viz__status"><strong data-role="step-label">Initial state</strong><span data-role="explanation">Choose an operation and prepare a trace.</span></div>
    ...
    render(isError = false) {
      ...
      this.root.querySelector('[data-role="stage"]').innerHTML = state.values.map((value, index) => {
      ...
      this.root.querySelector('.ds-viz__complexity').textContent = this.steps.length
        ? `Cost: ${state.moves === 0 ? 'O(1) so far' : `O(${state.moves}) moves, O(n) worst case`}`
        : 'Cost: —';
    }
  }

  function mountChapterDemo() {
    const path = window.location.pathname.replace(/\/$/, '');
    if (!path.endsWith('/04-insertion-deletion-and-shifting')) return;
    if (document.querySelector('[data-array-shift-viz]')) return;
